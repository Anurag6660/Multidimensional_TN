...
with open('singU%s.pkl'%t, 'wb') as f:
    pickle.dump(s, f)

with open('Ures_v%s.pkl'%t, 'wb') as f:
    pickle.dump(Ures_v, f)
...
for i in range(len(s_v)):
    s_v['%s'%i] = np.pad(s_v['%s'%i], (0,size_v-len(s_v['%s'%i])), 'constant', constant_values =0)

se_v = np.zeros((size_v,Ndim-1), dtype=np.float64)
for i in range(Ndim-1):
    se_v[:,i] = s_v['%s'%i]

np.savetxt('vse%s.txt'%t, se_v)
...
if DEBUG1:
  # prodU, MdU = modprop.recombine3D(Ures, Ndim, d, P)
   prodU, MdU = modprop.recombine3D(Ures, Ndim, d, P)
   prodV, MdV = modprop.recombine3D(Ures_v, Ndim, d, P_v)
   normc = np.linalg.norm(c-prodU)
   normV = np.linalg.norm(A1-prodV)
   print('normc = ',normc)
   print('normV = ',normV)
